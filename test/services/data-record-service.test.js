...
describe('DataRecordService', () => {
  ...
  describe('retrieveAllDSNodes', () => {
    const fs = require('fs');
    const path = require('path');
    const DataRecordService = require('../../services/data-record-service').DataRecordService;
    ...
    beforeEach(() => {
      ...
      // Patch the DataRecordService to use the mock DAO
      dataRecordService = new DataRecordService(
        {}, {}, {}, '', '', {}, {}, {}, {}, {}
      );
      dataRecordService.dataRecordDAO = mockDataRecordDAO;
      // Patch _saveNodesToTsv to avoid file system
      dataRecordService._saveNodesToTsv = jest.fn().mockResolvedValue(undefined);
    ...
    test('should not create directory if it exists', async () => {
      const aSubmission = { id: 'sub2', study: { studyAbbreviation: 'STUDY2' } };
      mockDataRecordDAO.distinct.mockResolvedValue(['file']);
      fs.existsSync.mockReturnValue(true);

      await dataRecordService.retrieveAllDSNodes(aSubmission);

      expect(fs.mkdirSync).not.toHaveBeenCalled();
      expect(dataRecordService._saveNodesToTsv).toHaveBeenCalledWith('file', 'sub2', expect.stringContaining('file.tsv'));
    });

    test('should handle no nodeTypes gracefully', async () => {
      const aSubmission = { id: 'sub3', study: { studyAbbreviation: 'STUDY3' } };
      mockDataRecordDAO.distinct.mockResolvedValue([]);
      fs.existsSync.mockReturnValue(true);

      await dataRecordService.retrieveAllDSNodes(aSubmission);

      expect(dataRecordService._saveNodesToTsv).not.toHaveBeenCalled();
    });
  });
});