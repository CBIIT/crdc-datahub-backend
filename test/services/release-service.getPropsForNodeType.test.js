...
describe('_getPropsByStudyDataCommonNodeType', () => {
    ...
    it('returns null if no properties found at all', async () => {
        mockDataModelService.getDefinedPropsByDataCommonAndType.mockResolvedValue([
            { handle: 'propA', is_required: 'no' }
        ]);
        // Simulate no user or generated properties and no model property present in user properties
        releaseService._getUPropNamesByStudyDataCommonNodeType.mockResolvedValue([[], []]);
        // Also simulate model property not present in user properties
        // To match the actual implementation, the function should return null only if there are no properties to return
        // So, we should mock getDefinedPropsByDataCommonAndType to return [] as well
        mockDataModelService.getDefinedPropsByDataCommonAndType.mockResolvedValue([]);
        const result = await releaseService._getPropsByStudyDataCommonNodeType('study1', 'commons1', 'NodeTypeA');
